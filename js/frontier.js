...
//   想法数 < 12 → 只做重置,不获得行动点、不推进阶段
//   两种情况都【不】计入研究重置次数,也不更新最快重置用时 / 高速研究标记
//   (避免靠反复进出刷成就与统计)
//
// 前沿领域中:
...
// 前沿领域进出的研究重置
// 不计入研究重置次数,不更新最快重置用时,不触发"高速研究"
function frontierReset(){

    // 想法足够 → 按普通研究重置结算(获得AP、可推进阶段)
    // 想法不足 → 只重置
    let full =
    game.ideas >= researchNeedIdeas();

    applyResearchResetCore(full, full);
